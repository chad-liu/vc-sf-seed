...
export default function ApplyForm() {
  ...
  // ---- 1. 申請資料 ----
  // 除備註外皆為必填；開班類別不能為「-」，人數不能為 0
  const REQUIRED_FIELDS: { key: keyof Apply; label: string }[] = [
  ...
  return (
    <div className="max-w-4xl space-y-8">
      <p className="text-sm text-red-400 text-center">7/31前將申請資料和經費預算輸入齊全、課程規畫檔案上傳，視為申請完成。</p>

      {/* 1. 申請資料：預設唯讀，按「修改」進入編輯模式 */}
      <div className="bg-blue-50 rounded-lg p-6">
        <SectionTitle>申請資料</SectionTitle>
        <div className="flex items-center gap-2 mb-3">
          <label className="w-40 text-sm text-gray-700 text-right flex-shrink-0">學年度</label>
          {editingApply ? (
            <input value={form.year} readOnly className={`${input} bg-gray-100`} />
          ) : (
            <span className="flex-1 text-sm text-gray-900 px-2 py-1.5 border-b border-gray-300 min-h-8">{form.year}</span>
          )}
        </div>
        <div className="flex items-center gap-2 mb-3">
        ...
        {field('申請目的', 'purpose', { placeholder: '必填(50字以內)' })}
        {field('課程內容', 'content', { placeholder: '必填(例如棒球、攝影)' })}
        {field('活動時段', 'activetime', { placeholder: '必填' })}
        {field('參加對象', 'activeobj', { placeholder: '必填' })}
        {field('受惠總人數', 'objnum', { type: 'number', placeholder: '必填' })}
        {field('其中弱勢學生人數', 'weaknum', { type: 'number', placeholder: '必填' })}
